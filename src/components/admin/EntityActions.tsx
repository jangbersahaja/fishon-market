...
export default function EntityActions({
  ...
  const entityPath = entityType === "category" ? "categories" : "tags";
  const queryParam = entityType === "category" ? "category" : "tag";

  const handleDelete = async () => {
    if (postCount > 0) {
      alert(
        `Cannot delete this ${entityLabel} because it has ${postCount} post${postCount !== 1 ? "s" : ""} associated with it. Please remove or reassign the posts first.`
      );
      return;
    }
    ...
    try {
      ...
      alert(`Failed to delete ${entityLabel}. Please try again.`);
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {postCount > 0 && (
        <Link
          href={`/blog?${queryParam}=${entitySlug}`}
          target="_blank"
      ...
      {/* Edit Button */}
      <Link
        href={`/admin/blog/${entityPath}/edit/${entityId}`}
