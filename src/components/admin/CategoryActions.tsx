...
export default function CategoryActions({
  ...
  const handleDelete = async () => {
    if (postCount > 0) {
      alert(
        `Cannot delete this category because it has ${postCount} post${postCount !== 1 ? "s" : ""} associated with it. Please remove or reassign the posts first.`
      );
      return;
    }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {/* Edit Button */}
      <Link
        href={`/admin/blog/categories/edit/${categoryId}`}
